...
- `fork_task` branches reuse the lead graph, message prefix, and sandbox for cache-friendly related parallel work. Forks never release that shared sandbox themselves; the terminal `ForkResult` returns its effective sandbox id in the parent `Command`, leaving the lead run as the sole lifecycle owner. In the public `RuntimeFeatures` factory, custom fork middleware is additive: the mandatory host publisher and nested-delegation guard always remain installed.
